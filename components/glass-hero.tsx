    ...
    >
      ...
      <header className="absolute top-0 left-0 right-0 z-30 pt-[max(2.5rem,env(safe-area-inset-top,0px))] px-[max(1.25rem,env(safe-area-inset-left,0px))] md:px-[max(5.6vw,2rem)] animate-nav-down pointer-events-auto">
        ...
        >
          ...
          {/* Right Actions: Mobile Theme Toggle & Let's Talk CTA */}
          <div className="flex items-center gap-2">
            <button
              onClick={toggleTheme}
              aria-label={`Switch to ${theme === "dark" ? "light" : "dark"} mode`}
              className="p-2.5 rounded-full bg-white/80 dark:bg-white/10 hover:bg-white dark:hover:bg-white/20 text-[#0a0f18] dark:text-white transition-colors border border-[#0a0f18]/10 dark:border-white/10 min-h-[44px] min-w-[44px] flex items-center justify-center cursor-pointer md:hidden"
            >
              {theme === "dark" ? (
                <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                  <circle cx="12" cy="12" r="5" />
            ...
            <a
              ...
              className="pill-button font-mono text-xs uppercase tracking-wider font-medium px-4 sm:px-5 py-2.5 rounded-full min-h-[44px] inline-flex items-center justify-center cursor-pointer text-[#0a0f18] dark:text-white"
            >
              Let&apos;s talk
            </a>
          </div>
